...
def create_pattern(size, species, offset=[0, 0], world=None):
    ...
    '''
    if world is None: world = np.zeros(size, dtype=int) #se non vuoi un ambiente pre-settato usa quello di default:  tutti zeri
    
    pattern = np.zeros(size, dtype=int)
    
    start_row, start_col = offset[0] + size[0] // 2 - 1, offset[1] + size[1] // 2 - 1 #posizione inizale del pattern
